...
- **Rollout complete (July 2026, branch `fable-frontend-redesign`)**: every remaining page has been migrated to the `ss/` kit — Run/track, Run history, Progress, Events, Social/Community (incl. CommunityDetail + CreateCommunity), Profile/Account, User profiles, Goals (SetGoal), Share, Challenges, Rewards, Notifications, Subscription, AI Profiling/Profile onboarding, Auth (login/register/forgot/reset), Landing splash, and the full 15-tab **Admin** portal. Design gates hold app-wide (0 emoji in code, tokens only). Live click-through verified against a local backend; fixes landed for stalled nested `AnimatePresence mode="wait"` transitions, invisible range-slider rails, `.ss-btn{flex:1}` blowups in column layouts, missing scroll reset on route change, and the Backup tab's string-concatenated row counts. LandingPage (`/join`, `/founding`) was deliberately left on its own styling (out of preview scope).
