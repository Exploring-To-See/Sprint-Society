...
router.post('/comments/:id/hide', async (req: AuthRequest, res: Response) => {
  const comment = await db.queryOne('SELECT * FROM comments WHERE id = $1', [req.params.id]) as any;
  if (!comment) return res.status(404).json({ error: 'Comment not found' });

  await db.execute('DELETE FROM comments WHERE id = $1', [req.params.id]);
...
router.post('/posts/:id/hide', async (req: AuthRequest, res: Response) => {
  const post = await db.queryOne('SELECT * FROM community_posts WHERE id = $1', [req.params.id]) as any;
  if (!post) return res.status(404).json({ error: 'Post not found' });

  await db.execute('DELETE FROM community_posts WHERE id = $1', [req.params.id]);
...
router.post('/users/:id/warn', async (req: AuthRequest, res: Response) => {
  const { reason } = req.body;
  const userId = parseInt(req.params.id);

  const user = await db.queryOne('SELECT id, name FROM users WHERE id = $1', [userId]) as any;
...
router.post('/users/:id/ban', async (req: AuthRequest, res: Response) => {
  ...
  const user = await db.queryOne('SELECT id, name, role FROM users WHERE id = $1', [userId]) as any;
  if (!user) return res.status(404).json({ error: 'User not found' });
...
router.get('/chat-messages', async (req: AuthRequest, res: Response) => {
...
router.delete('/chat-messages/:id', async (req: AuthRequest, res: Response) => {
  const msgId = parseInt(req.params.id);
  const msg = await db.queryOne('SELECT * FROM community_chat_messages WHERE id = $1', [msgId]) as any;
  if (!msg) return res.status(404).json({ error: 'Message not found' });

  await db.execute('DELETE FROM community_chat_messages WHERE id = $1', [msgId]);
  await logAuditAction(req.userId!, 'delete_chat_message', 'chat_message', msgId, JSON.stringify({ body: msg.body, community_id: msg.community_id }));
