...
router.get('/', async (req: AuthRequest, res: Response) => {
  ...
  if (type && type !== 'all') {
    where += ` AND e.event_type = $${paramIndex++}`;
    params.push(type);
  }
  if (from) {
    where += ` AND e.date >= $${paramIndex++}`;
    params.push(from);
  }
  if (to) {
    where += ` AND e.date <= $${paramIndex++}`;
  ...
  if (eventIds.length > 0) {
    const ePlaceholders = eventIds.map((_: any, i: number) => `$${i + 2}`).join(',');
    const friendsGoing = await db.query(`
      SELECT er.event_id, u.id, u.name, u.profile_image_url
      FROM event_rsvps er
      JOIN users u ON er.user_id = u.id
      JOIN follows f ON f.following_id = er.user_id AND f.follower_id = $1
      WHERE er.event_id IN (${ePlaceholders}) AND er.status = 'going'
    `, [req.userId, ...eventIds]) as any[];
...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  ...
  const attendees = await db.query(`
    ...
    FROM event_rsvps er
    JOIN users u ON er.user_id = u.id
    WHERE er.event_id = $1 AND er.status IN ('going', 'maybe')
...
router.post('/:id/rsvp', async (req: AuthRequest, res: Response) => {
  ...
  if (status === 'going' && event.max_attendees) {
    const currentCount = (await db.queryOne('SELECT COUNT(*) as c FROM event_rsvps WHERE event_id = $1 AND status = $2', [eventId, 'going']) as any).c;
    if (parseInt(currentCount) >= event.max_attendees) {
      return res.status(409).json({ error: 'Event is full' });
    }
  }

  const existing = await db.queryOne('SELECT * FROM event_rsvps WHERE event_id = $1 AND user_id = $2', [eventId, req.userId]) as any;
  ...
  res.json({ success: true, attendee_count: parseInt(attendee_count), user_rsvp: status });
});

// DELETE /events/:id/rsvp — cancel RSVP
router.delete('/:id/rsvp', async (req: AuthRequest, res: Response) => {
  const eventId = parseInt(req.params.id);
  await db.execute('DELETE FROM event_rsvps WHERE event_id = $1 AND user_id = $2', [eventId, req.userId]);
  const attendee_count = (await db.queryOne('SELECT COUNT(*) as c FROM event_rsvps WHERE event_id = $1 AND status = $2', [eventId, 'going']) as any).c;
  res.json({ success: true, attendee_count: parseInt(attendee_count), user_rsvp: null });
});
...
router.post('/:id/comments', async (req: AuthRequest, res: Response) => {
  ...
  const eventId = parseInt(req.params.id);
  const event = await db.queryOne('SELECT id FROM events WHERE id = $1', [eventId]) as any;
  if (!event) return res.status(404).json({ error: 'Event not found' });

  const result = await db.execute('INSERT INTO event_comments (event_id, user_id, body) VALUES ($1, $2, $3) RETURNING id', [eventId, req.userId, body.trim()]);

  const comment = await db.queryOne(`
    SELECT ec.*, u.name as user_name, u.profile_image_url
    FROM event_comments ec JOIN users u ON ec.user_id = u.id WHERE ec.id = $1
  `, [result.rows[0]?.id]) as any;

  res.json(comment);
});

// GET /events/:id/my-awards — user's awards from this event
router.get('/:id/my-awards', async (req: AuthRequest, res: Response) => {
  const awards = await db.query(`
    SELECT * FROM event_awards WHERE event_id = $1 AND user_id = $2 ORDER BY rank_position ASC NULLS LAST
  `, [parseInt(req.params.id), req.userId]) as any[];

  const activity = await db.queryOne(`
    SELECT * FROM activities WHERE user_id = $1 AND date(start_date) = (SELECT date FROM events WHERE id = $2)
...
router.post('/:id/checkin', async (req: AuthRequest, res: Response) => {
  ...
  const existing = await db.queryOne('SELECT id FROM event_checkins WHERE event_id = $1 AND user_id = $2', [eventId, req.userId]) as any;
  if (existing) return res.json({ success: true, already_checked_in: true, message: 'Already checked in!' });

  await db.execute('INSERT INTO event_checkins (event_id, user_id) VALUES ($1, $2)', [eventId, req.userId]);
  ...
  await awardXP(req.userId!, 10, 'event_attendance', `Attended: ${event.title}`);
...
router.get('/:id/recap', async (req: AuthRequest, res: Response) => {
  ...
  const placeholders = attendeeIds.map((_: any, i: number) => `$${i + 1}`).join(',');
  ...
  const runs = await db.query(`
    ...
    AND DATE(a.start_date) = $${attendeeIds.length + 1}
    ORDER BY a.average_pace_per_km ASC
  `, [...attendeeIds, eventDate]) as any[];
